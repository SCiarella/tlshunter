...
print(f"\n*** SHAP analysis for {In_label}")

try:
    shapdir = f"./output_ML/{In_label}"
    os.makedirs(shapdir)
except OSError:
    pass
...
X_c = data_classifier.drop(columns=["i", "j", "conf", class_name])
Y_c = data_classifier[class_name]
...
# Load the classifier
model_path = f"MLmodel/classification-{In_label}"
...
print(
    f"\n**** Calculating the shap parameters for the predictor using {N_baseline} samples as baseline and {N_SHAP} test points",
)
...
print(
    f"\n**** Calculating the shap parameters for the classifier using {N_baseline} samples as baseline and {N_SHAP} test points",
)
