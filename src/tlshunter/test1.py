...
if __name__ == "__main__":
    ...
    validation_set = pd.read_feather(
    ...
    validation_set = validation_set.sort_values("target_feature",
                                                ascending=False)
    validation_set = validation_set[validation_set["target_feature"] <
                                    maximum_value_tg_feature]
    validation_set_nolab = validation_set.drop(
        columns=["i", "j", "conf", "target_feature", "10tominus_tg"])
    validation_set_nolab = TabularDataset(validation_set_nolab)
    y_true_val = np.power(10,
                          -validation_set["10tominus_tg"])  # values to predict
    ...
    print("\n\nPerformances:")
    perf = predictor.evaluate_predictions(y_true=y_true_val,
                                          y_pred=y_pred_by_AI,
    ...
    cb.set_label("counts")
    plt.tight_layout()
    plt.savefig(f"output_ML/{In_label}/{In_label}-predicted_vs_true.png",
                dpi=150)
    plt.close()
