...
def process_chunk(
    ...
    """Process a chunk of data to find corresponding input pairs based on given criteria.
    ...
    -------
    pd.DataFrame
        Processed data containing matched pairs.
    """
...
def prepare_data(
    ...
    """Prepare data for training by loading necessary files and checking for new calculations.
...
def construct_pairs(
    ...
    """Construct pairs of data for training by processing chunks in parallel.
...
def prepare_training_data(
    ...
    """Prepare training and validation data by combining new and pretraining data.
    ...
    training_set = pd.concat(
        [goodpairs_df.iloc[:Ntrain], badpairs_df.iloc[:Ntrain]]).sample(
            frac=1,
            random_state=20,
            ignore_index=True,
        )
    validation_set = pd.concat(
        [goodpairs_df.iloc[Ntrain:N], badpairs_df.iloc[Ntrain:N]]).sample(
            frac=1,
            random_state=20,
            ignore_index=True,
        )
