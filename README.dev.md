# `tlshunter` developer documentation
...
# install tlshunter as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `tlshunter`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d tlshunter.XXXXXX)
git clone git@github.com:SCiarella/tlshunter .
...
Visit
[https://test.pypi.org/project/tlshunter](https://test.pypi.org/project/tlshunter)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d tlshunter-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple tlshunter
```
...
Don't forget to also make a [release on GitHub](https://github.com/SCiarella/tlshunter/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.